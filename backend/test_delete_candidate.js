...
async function testDeleteCandidate() {
  try {
    console.log('\n🧪 PROBANDO ELIMINACIÓN DE CANDIDATO\n');
    ...
    const evalCountBefore = await pool.query(
      `SELECT COUNT(*) as count FROM evaluations WHERE candidate_vacancy_id IN (
    ...
    console.log(`   Evaluaciones: ${evalCountBefore.rows[0].count}`);
    console.log(`   Respuestas de examen: ${ansCountBefore.rows[0].count}\n`);

    // Eliminar candidato vía API
