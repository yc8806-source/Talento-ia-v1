...
function generatePDF(results, candidateName, cvId) {
  ...
  try {
    ...
    const overallLevel = overall.level || calculateLevel(overall.percentage);
    ...
    doc.fontSize(11).font('Helvetica').text(`${overall.score}/${overall.maxScore} pts`, 70, yPos + 35);
    doc.fontSize(11).font('Helvetica').text(`Nivel: ${overallLevel}`, 300, yPos + 5);

    yPos += 65;

    // Competencias
    doc.fontSize(12).font('Helvetica-Bold').fillColor('#1A237E').text('Perfil de Competencias', 50, yPos);
    yPos += 20;
...
async function main() {
  ...
  for (let i = 0; i < cvIds.length; i++) {
    ...
    try {
      ...
      // Generar PDF
      const pdfData = await generatePDF(results, name, cvId);

      console.log(`   ✅ ${pdfData.filename}`);
      console.log(`   📊 ${results.overall.percentage}%\n`);
