...
// Preguntas del TPL-80 organizadas por competencia
const competencies = [
  {
  ...
  try {
    ...
    if (existingExam.rows.length > 0) {
      ...
      const examResult = await client.query(
        ...
        [
          'TEST DE PERSONALIDAD LABORAL (TPL-80)',
          'Evaluación de 10 competencias laborales con escala Likert (1-5). 80 preguntas que miden: Responsabilidad, Orientación al Logro, Trabajo Bajo Presión, Adaptabilidad, Trabajo en Equipo, Orientación al Cliente, Integridad, Inteligencia Emocional, Iniciativa y Resiliencia.',
          60,
          'personality'
        ]
      );
      ...
      for (const q of competency.questions) {
        ...
        const questionResult = await client.query(
          `INSERT INTO questions (title, type, competency_id, is_inverse)
           VALUES ($1, $2, $3, $4)
           RETURNING id`,
          [q.text, 'likert_5', competency.id, q.inverse]
        );
